...
export async function POST(request: NextRequest) {
  ...
  try {
    logger.debug('🖼️ All users heatmap render API called');
    ...
    const { year, format, width, height } = validated;

    logger.debug('🔍 Rendering all users heatmap:', { year, format, width, height });

    // 3. Authorization - only admins can export all users' heatmaps
    const userRoles = decoded.roles || [];
    const isAdmin = userRoles.includes('SUPER_ADMIN') || userRoles.includes('ADMIN');

    if (!isAdmin) {
      logger.security('Unauthorized all-users heatmap export attempt', {
        userId: decoded.userId,
      });
      return NextResponse.json(
        { error: 'Only administrators can export all users\' heatmaps' },
        { status: 403 }
      );
    }

    // 4. Fetch flight data for all users
    ...
    logger.info('All users heatmap rendered successfully', {
      year,
      format,
    ...
    return new NextResponse(screenshot, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename=\"all-flights-heatmap-${year}.${format}\"`,
    ...
    logger.error('All users heatmap render failed', {
