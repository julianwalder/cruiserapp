...
async function fixAllIdentifiedForeignKeys() {
  console.log('🔧 Fixing all identified foreign key constraint issues...');
  
  try {
    // Based on the query results provided
    ...
    // Step 4: Check for any other tables that might have aircraft ID references
    console.log('-- Check for any other tables with aircraft ID references:');
    console.log(`
      SELECT 
        table_name,
        column_name,
      ...
      AND table_name NOT IN ('aircraft', 'aircraft_hobbs', 'flight_logs')
    ...
    console.log('');
    console.log('⚠️ Note: If you still get the fleet_management error after this,');
    console.log('   it might be from a table that wasn\'t captured in the foreign key query.');
    console.log('   Run the verification queries to identify any remaining issues.');
