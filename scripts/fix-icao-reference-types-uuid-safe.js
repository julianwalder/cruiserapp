...
async function fixIcaoReferenceTypesUUIDSafe() {
  console.log('🔧 Converting ICAO reference types from text IDs to UUIDs (safe approach)...');
  ...
  try {
    ...
    // Step 4: Update aircraft references first (to avoid foreign key constraint issues)
    console.log('\n🔄 Step 4: Updating aircraft references...');
    ...
    // Step 5: Now update ICAO reference types one by one
    console.log('\n🔄 Step 5: Updating ICAO reference types...');
    let updatedIcaoCount = 0;

    for (const icaoType of newIcaoTypes) {
      try {
        // First, insert the new record with UUID
        ...
        if (insertError) {
          console.error(`❌ Error inserting new ICAO type ${icaoType.newId}:`, insertError);
          continue;
        }
        ...
        const { error: deleteError } = await supabase
          .from('icao_reference_type')
          .delete()
          .eq('id', icaoType.oldId);

        if (deleteError) {
          console.error(`❌ Error deleting old ICAO type ${icaoType.oldId}:`, deleteError);
          // Try to clean up the new record if deletion failed
          await supabase
            .from('icao_reference_type')
            .delete()
            .eq('id', icaoType.newId);
          continue;
        }

        updatedIcaoCount++;
        console.log(`✅ Updated ICAO type: ${icaoType.manufacturer} ${icaoType.model} (${icaoType.oldId} → ${icaoType.newId})`);

      } catch (error) {
        console.error(`❌ Error processing ICAO type ${icaoType.oldId}:`, error);
      }
    }
