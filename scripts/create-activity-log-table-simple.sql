-- Create activity_log table for tracking system events (Simplified version)
...
-- Create RLS policies (simplified - no type casting needed)
    ...
    FOR SELECT USING (
        EXISTS (
            ...
            AND EXISTS (
                ...
                WHERE ur."userId" = auth.uid() AND r.name IN ('ADMIN', 'SUPER_ADMIN')
            )
        )
    );
