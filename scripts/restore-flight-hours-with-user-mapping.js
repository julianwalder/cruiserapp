...
async function restoreFlightHoursWithUserMapping() {
  console.log('🔧 Restoring flight_hours table with proper user mapping...');
  ...
  try {
    ...
    // Get current invoice_clients to map user_id
    const { data: currentInvoiceClients, error: currentInvoiceClientsError } = await supabase
      .from('invoice_clients')
      .select('invoice_id, user_id');

    if (currentInvoiceClientsError) {
      console.error('❌ Error fetching current invoice clients:', currentInvoiceClientsError);
      return;
    }
    ...
    console.log(`✅ Found ${currentInvoiceClients.length} current invoice clients`);

    // Step 2: Create mappings
    console.log('📊 Step 2: Creating mappings...');
    ...
    // Create mapping from invoice_id to user_id
    const invoiceToUserId = new Map();
    currentInvoiceClients.forEach(client => {
      if (client.user_id) {
        invoiceToUserId.set(client.invoice_id, client.user_id);
      }
    });

    console.log(`📋 Created mapping for ${invoiceToUserId.size} invoices with user relationships`);
    ...
    for (const backupFlightHour of backupFlightHours) {
      try {
        ...
        // Get the user_id for this invoice
        const userId = invoiceToUserId.get(currentInvoiceId);
        
        if (!userId) {
          console.log(`   ⚠️  No user found for invoice ${smartbillId} (${currentInvoiceId})`);
          skippedCount++;
          continue;
        }

        console.log(`📝 Processing ${smartbillId} -> Flight Hours Record (User: ${userId})`);
        ...
        // Insert the flight hours record with the new invoice UUID and user_id
        const { error: insertError } = await supabase
          .from('flight_hours')
          .insert({
            invoice_id: currentInvoiceId,
            user_id: userId, // Add the user_id from invoice_clients
...
// Run the function
restoreFlightHoursWithUserMapping(); 