...
async function checkRecentVeriffValidations() {
  console.log('🔍 Checking recent Veriff validations...\n');

  try {
    // Get users with Veriff data, ordered by most recent
    ...
    if (!users || users.length === 0) {
      console.log('📭 No users with Veriff validations found');
      return;
    }

    console.log(`📊 Found ${users.length} users with Veriff validations:\n`);

    users.forEach((user, index) => {
      console.log(`👤 User ${index + 1}: ${user.firstName} ${user.lastName} (${user.email})`);
      console.log(`   ID: ${user.id}`);
      console.log(`   Status: ${user.veriffStatus || 'Unknown'}`);
