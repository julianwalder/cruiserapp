...
# Minimal Scripts Cleanup - Keep only truly essential scripts
# Based on actual code analysis, only keep what's referenced

echo "🧹 Starting MINIMAL scripts cleanup..."
echo "⚠️  This will keep only the truly essential scripts!"

# ONLY these scripts are actually needed based on code analysis
ESSENTIAL_SCRIPTS=(
    # Recent and useful
    ...
    # Core database schemas (most recent/complete versions only)
...
echo "✅ Essential files to keep: $ESSENTIAL_COUNT"

# Create a temporary file to store files to delete
TEMP_DELETE_LIST="/tmp/scripts_to_delete_minimal.txt"
> "$TEMP_DELETE_LIST"
...
echo ""
echo "⚠️  MINIMAL CLEANUP: This will delete $FILES_TO_DELETE files from the scripts directory."
echo "✅ Only $ESSENTIAL_COUNT essential files will be preserved."
echo "🚨 This removes 95%+ of the scripts folder!"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo "🗑️  Files deleted: $FILES_TO_DELETE"
    echo "✅ Essential files preserved: $ESSENTIAL_COUNT"
    echo "📈 Cleanup percentage: $(( (FILES_TO_DELETE * 100) / TOTAL_FILES ))%"
else
    echo "❌ Minimal cleanup cancelled."
    rm "$TEMP_DELETE_LIST"
fi

echo "🧹 Minimal scripts cleanup finished."
