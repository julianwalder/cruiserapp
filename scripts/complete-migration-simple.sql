-- Complete UUID Migration Script (Simplified Version)
-- This script completes the migration with the current database state
...
-- Step 4: Handle user_company_relationships - convert existing user_id to UUID
-- First, add a new UUID column
ALTER TABLE user_company_relationships 
ADD COLUMN user_id_uuid UUID;

-- Update the new column with UUID values from users table
