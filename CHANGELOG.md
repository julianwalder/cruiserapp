...
### Changed
- **Flight Type Defaults**
  - Updated default flight type from "SCHOOL" to "INVOICED" for pilots and other users
  - Students still default to "SCHOOL" flight type and cannot change it
  - Improved role-based flight type defaults for better user experience

### Enhanced
- **Charter Flight Payer Selection**
  - Enhanced charter flight payer selection to show all users in the system
  - Added `fetchAllUsers` function to get comprehensive user list for payer selection
  - Improved user experience when selecting payers for charter flights
  - Fallback to pilots + instructors if user lacks permission to fetch all users
